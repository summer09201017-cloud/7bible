// ⚠ 共用 core 檔(0809 鐵則):Strong's 原文編號(FHL 信望愛資料源)。
// 目前只有 7bible 用;若 8biblesearch 也要加原文功能,先把本檔收進 skills 合輯當正本,
// 再垂直搬運過去——**勿複製後就地分岔**。零相依、無 React,可整段貼進單檔站。
//
...
//   {…} = 譯者補字或未譯出的原文字。engs 參數實測會回錯書卷,一律用 chineses。

const QB_URL = 'https://bible.fhl.net/json/qb.php';
const SD_URL = 'https://bible.fhl.net/json/sd.php';
const DICT_LS_KEY = 'sevenbible-strongs-dict-v1'; // 字典快取(重複查詢即離線可用);是快取不是使用者資料,刻意不進備份鏈
const DICT_CAP = 200;
...
export function parseStrongsText(raw) {
  ...
  while ((m = re.exec(String(raw || ''))) !== null) {
    if (m[4] !== undefined) {
      // 依標點切開;FHL 的編號掛在「上一個標籤之後的整段」,語意上屬於最後一個詞塊
...
// chineses = FHL 中文書卷縮寫(bookMap.names[0]:創/出/…/約/啟)
