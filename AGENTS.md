...
- **健康度**：`npm run lint` 綠、`npx vite build` 成功、無自動化測試（沒有 `npm test`）。SW 版本 `bible-app-v23`(0809)。
...
| `src/lib/strongs.js` | 共用 core（0809 鐵則）：Strong's 原文編號——FHL qb.php 帶標和合本解析 + sd.php 中文字典（localStorage LRU 快取）。零 React，可垂直搬運到 8bible，勿就地分岔。 |
...
- AI 釋義 (Codex API), 靈修日記, 統計圖表, Web Worker 搜尋索引, 語音輸入搜尋. (Strong's 原文編號 v23、上下文預覽 v22 已完成)
